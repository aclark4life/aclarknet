...
from db.models import Time, Invoice, Project, Client
...
class TimeViewAdminFieldsTest(TestCase):
    ...
    def test_time_update_view_passes_user_to_form(self):
        ...
        time_entry = Time.objects.create(
            user=self.regular_user, hours=8.0, description="Test work"
        )
    ...
    def test_time_update_view_admin_user_gets_all_fields(self):
        ...
        # Create a time entry
        time_entry = Time.objects.create(
            user=self.admin_user, hours=8.0, description="Test work"
        )
    ...
    def test_time_create_view_prepopulates_project_from_invoice(self):
        """Test that project is pre-populated when creating time entry from invoice."""
        # Create test data
        ...
        # Create view with invoice_id parameter
        view = TimeCreateView()
        request = self.factory.get(f"/time/create/?invoice_id={invoice.pk}")
        request.user = self.admin_user
        view.request = request
        view.object = None
        ...
        # Project should be pre-populated from invoice
        self.assertEqual(form.initial.get("project"), project)
        self.assertEqual(form.initial.get("invoice"), str(invoice.pk))
