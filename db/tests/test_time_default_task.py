...
Time.save() (see db/models.py) auto-assigns a task when none is set:
project.default_task if the project has one, otherwise the global
default from Task.get_default_task() (name="Software Development",
rate=187.50). There is no per-user default task (no Profile model
exists in this project), so tests for that were removed.
"""

from django.contrib.auth import get_user_model
from django.test import TestCase

from db.models import Task, Time, Project

User = get_user_model()

DEFAULT_TASK_NAME = "Software Development"
...
class TimeDefaultTaskTest(TestCase):
    ...
    def test_time_entry_gets_default_task_when_none_specified(self):
        ...
        self.assertEqual(time_entry.task.name, DEFAULT_TASK_NAME)
    ...
    def test_time_entry_keeps_custom_task_when_specified(self):
        ...
        self.assertNotEqual(time_entry.task.name, DEFAULT_TASK_NAME)
    ...
    def test_default_task_is_reused(self):
        ...
        self.assertEqual(time_entry1.task.name, DEFAULT_TASK_NAME)

        # Verify only one default task exists
        default_tasks = Task.objects.filter(name=DEFAULT_TASK_NAME)
...
class TimeProjectDefaultTaskTest(TestCase):
    ...
    def test_explicit_task_overrides_project_default(self):
        """Test that an explicitly set task takes priority over project default.

        Time.save() only auto-assigns a task when self.task is falsy, so an
        explicitly provided task is always kept as-is.
        """
        custom_task = Task.objects.create(
            name="Custom Task",
            rate=200.00,
        ...
        # Should keep the explicitly set task, not the project's default
        self.assertEqual(time_entry.task, custom_task)
