...
## Overview
This feature adds the ability to designate specific clients to be displayed on the public `/clients/` page and categorize them into groups like Government, Non-profit, Private Sector, etc.
...
- Added `category` field with predefined choices:
  ...
  - Healthcare
  - Other
- The `ClientCategory` enum is defined as a nested class in the `Client` model
...
- The migration is compatible with the MongoDB backend used by the project
...
- Enhanced `ClientAdmin` with:
  - `list_display` showing name, company, category, and featured status
...
- Non-featured clients (featured=False) will NOT appear on the public clients page
- Clients without a category will be grouped under "Other"
- The feature is backward compatible - existing clients default to featured=False
